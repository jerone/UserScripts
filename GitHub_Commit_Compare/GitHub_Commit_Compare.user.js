...
(function () {
  ...
  function addButton() {
	var e;
	if ((e = document.querySelector('.file-navigation'))) {

	  const r = e.querySelector('#GitHubCommitCompareGroup');
	  if (r) {
	  ...
	  c.addEventListener('change', function () {
		var bb = document.querySelectorAll('.GitHubCommitCompareButtonAB');
		if (this.checked) {
		  if (bb.length === 0) {
			addCompareButtons();
		  } else {
	  ...
	  `;
	  l.setAttribute('aria-label', 'Show commit compare checkboxes');
	  l.appendChild(c);
	  ...
	  a.setAttribute('aria-label', 'Show commit compare checkboxes');
	  a.style.cssText = `border-top-left-radius: 0;
  ...
  function addCompareButtons() {
	function updateCheckboxes() {
	  ...
	  Array.from(compares).forEach(function (compare) {
		var compareA = compare.querySelector('[name="GitHubCommitCompareButtonA"]');
		var compareB = compare.querySelector('[name="GitHubCommitCompareButtonB"]');
	...
	function updateCompare() {
	  ...
	  a.setAttribute('href', `https://github.com/greasemonkey/greasemonkey/compare/${hashA}...${hashB}`);
	...
	Array.from(commits).forEach(function (item, index) {
	  ...
	  c1.type = 'radio';
	  c1.addEventListener('change', updateCheckboxes);
	  ...
	  c2.type = 'radio';
	  c2.addEventListener('change', updateCheckboxes);
	...
	updateCheckboxes(); // Init update checkboxes.
  }
